...
#pragma ivdep       
        for(int j = 0;j<nRF1;++j){
            data[i*stride+1+2*nBeam+j*3] = data[iprv*stride+1+2*nBeam+j*3]*k[j*5]+data[iprv*stride+1+2*nBeam+j*3+1]*k[j*5+1]+data[iprv*stride+1]*k[j*5+2]+data[iprv*stride+2]*k[j*5+3]+data[iprv*stride+1+2*nBeam+j*3+2]*k[j*5+4];
...
#pragma ivdep
        for(int j = nRF1;j<nRF1+nRFc;++j){
            data[i*stride+1+2*nBeam+j*3] = data[iprv*stride+1+2*nBeam+j*3]*k[j*5]+data[iprv*stride+1+2*nBeam+j*3+1]*k[j*5+1]+(data[iprv*stride+1]+data[iprv*stride+3])*k[j*5+2]+(data[iprv*stride+2]+data[iprv*stride+4])*k[j*5+3]+data[iprv*stride+1+2*nBeam+j*3+2]*k[j*5+4];
            data[i*stride+1+2*nBeam+j*3+2] = data[iprv*stride+1+2*nBeam+j*3+2]+dt*data[i*stride+1+2*nBeam+j*3];
        }
#pragma ivdep
        for(int j = nRF1+nRFc;j<nRF1+nRFc+nRF2;++j){
            data[i*stride+1+2*nBeam+j*3] = data[iprv*stride+1+2*nBeam+j*3]*k[j*5]+data[iprv*stride+1+2*nBeam+j*3+1]*k[j*5+1]+data[iprv*stride+3]*k[j*5+2]+data[iprv*stride+4]*k[j*5+3]+data[iprv*stride+1+2*nBeam+j*3+2]*k[j*5+4];
            data[i*stride+1+2*nBeam+j*3+2] = data[iprv*stride+1+2*nBeam+j*3+2]+dt*data[i*stride+1+2*nBeam+j*3];
        }
...
#pragma ivdep
    ...
    delete[] Rinv;
    /*
    std::cout<<(I_I_ref[0]+(V_I_ref[0]-I[0])*Rinv[0]*gii[0])<<','<<I_I_ref[0]<<std::endl;
    std::cout<<V_I_ref[0]<<','<<I[0]<<std::endl;
    std::cout<<(I_I_ref[1]+(V_I_ref[1]-I[1])*Rinv[1]*gii[1])<<','<<I_I_ref[0]<<std::endl;
    std::cout<<V_I_ref[1]<<','<<I[1]<<std::endl;
    */
...
/*
void update_index_neighbor(double* bunch, double* weight, double dt, double Trev, int fill_count, 
                            int nPar, int totBin, int shift, int nBunch,int nBin,int beam_shift,int nBucket,
                            int* index, int* neighbor, int* parFlag){
    double dtInver = 1/dt;
    int totN = fill_count*nPar;
    // first totN data in bunch array is 'time' info of the particles.
...
#pragma ivdep
    for(int i = 0;i<totN;++i){
        ...
        index[i] += shift;
        neighbor[i] += shift;
        // if the particle is out side of the bucket, flag it as lost
        int ibunch = (i/nPar)%nBunch;
        int ibeam = i/(nPar*nBunch);
        //std::cout<<i<<','<<ibunch<<std::endl;
        parFlag[i] = (index[i]-shift>=(ibunch*3+ibeam*(beam_shift+0.5))*nBin)&(index[i]-shift<=(ibunch*3+ibeam*(beam_shift+0.5)+1)*nBin)?1:0;
#if 0
...
/*
void update_M1(double* particles, double* M1, double* temp, int nPar, int fill_count,int* Npar_survive,int* parFlag){
    double nParInver = 1.0/nPar;
    for(int i = 0;i<fill_count;++i){
        Npar_survive[i] = std::accumulate(parFlag+i*nPar,parFlag+(i+1)*nPar,0.0);
    }
    for(int i = 0;i<fill_count*nPar;++i){
        temp[i] = particles[i]*parFlag[i];
        temp[fill_count*nPar+i] = particles[fill_count*nPar+i]*parFlag[i];
    }
    for(int i = 0;i<fill_count;++i){
        M1[i] = std::accumulate(temp+i*nPar,temp+(i+1)*nPar,0.0)/Npar_survive[i];
        M1[fill_count+i] = std::accumulate(temp+(fill_count+i)*nPar,temp+(fill_count+i+1)*nPar,0.0)/Npar_survive[i];
    }
}
*/
void update_M2(double* particles, double* M1, double* M2, int nPar, int fill_count){
    ...
    /*
    std::cout<<"RMS t of first bunch is : "<<sqrt(M2[0])<<std::endl;
    std::cout<<"RMS Gamma of first bunch is : "<<sqrt(M2[fill_count])<<std::endl;
    std::cout<<"Correlation of first bunch is : "<<(M2[fill_count*2])<<std::endl;
    std::cout<<"RMS Emittance of first bunch is : "<<pi*sqrt(M2[0]*M2[fill_count]-M2[fill_count*2]*M2[fill_count*2])*E0<<std::endl;
    std::cout<<"95% Emittance of first bunch per neucleus is : "<<6*pi*sqrt(M2[0]*M2[fill_count]-M2[fill_count*2]*M2[fill_count*2])*E0p<<std::endl;
    */
}
...
#pragma ivdep
        ...
        for(int i = 0;i<nTot/nBeam;++i){
            ...
            //std::cout<<i<<','<<data[par_idx*stride+1+2*nBeam+j*3]<<','<<(data[par_neighbor*stride+1+2*nBeam+j*3]-data[par_idx*stride+1+2*nBeam+j*3])<<','<<fabs(weight[i])<<','<<particle_coords[nTot+i]<<std::endl;
        }
        //ANNOTATE_SITE_END();
    }
...
void updateParticle_e(double* particle_coords, double* data, double* weight, double* M1,
                    ...
                    int fill_count, int nPar, int nRF, int nHOM,
                    int* index, int* neighbor){
// for electron
    int nTot = fill_count*nPar;
    int stride = 3+3*nRF+2*nHOM;
    double dtInver = 1/dt;
    double Gamma0Inver = 1/Gamma0;
    double nParInv = 1/nPar; 
    // gamma 
    int j = 0;
//#pragma omp parallel for schedule(static,nPar) num_threads(OMP_NUM_THREADS) private(j)
    for(int j = 0;j<nRF;++j){
#pragma omp parallel for schedule(static,nPar) num_threads(OMP_NUM_THREADS)
#pragma ivdep
        for(int i = 0;i<nTot;++i){
            ...
            int par_neighbor = neighbor[i];
            double V = 0;
            V = data[par_idx*stride+3+j*3]+(data[par_neighbor*stride+3+j*3]-data[par_idx*stride+3+j*3])*dtInver*fabs(weight[i]);
            particle_coords[nTot+i] = particle_coords[nTot+i]+qoverme*(V-Urad0)*c_inver*c_inver*qoverme_on;
        }
    }
// minic the phase loop dampping 
#pragma omp parallel for schedule(static,nPar) num_threads(OMP_NUM_THREADS)
#pragma ivdep 
    for(int i = 0;i<nTot;++i){
...
#pragma ivdep 
    for(int i = 0;i<nTot;++i){
        particle_coords[nTot+i] -= qoverme_on*(coeff_decay_long*(particle_coords[nTot+i]-Gamma0)+coeff_excite_long*(2*double(rand())/double(RAND_MAX)-1));//+coeff_excite_long; // damping term
    }
...
void updateIb(double* particle_coords, double* data, double* weight, 
    ...
    for(int j = 0;j<nBeam;++j){
        for(int i = j*nBunchPerBeam*nPar; i<(j+1)*nBunchPerBeam*nPar ; ++i){
            data[(index[i])*stride+j*2+1] += q[j]*(1-fabs(weight[i]*dtInver))*dtInver;
            data[(neighbor[i])*stride+j*2+1] += q[j]*fabs(weight[i]*dtInver)*dtInver;
            data[(index[i]+1)*stride+j*2+2] += q[j]*(1-fabs(weight[i]*dtInver))*dtInver;
            data[(neighbor[i]+1)*stride+j*2+2] += q[j]*fabs(weight[i]*dtInver)*dtInver;
        }
    }
}
//void updateFrq(int rfIdx,double* k,double* omegac,double* C, double* Leff,)
...
void get_init_t0s(double* t0s,double dT, double t0, double Trev, int nBeam, int nBunch, int shift,int nBucket){
...
int main(){
    ...
    int n_q_ramp = input1.generalPara["n_q_ramp"];
    ...
    int detune_slow_factor = input1.generalPara["detune_slow_factor"];
    int n_detune_ramp_tot = input1.generalPara["n_detune_ramp_tot"];// the turn we stop tuning.
    int n_fill = input1.generalPara["n_fill"]; // turn number when we start filling.
    int fill_step = input1.bunchPara["fill_step"]; // fill pattern
    int fill_count = 0;
    int nPar = input1.bunchPara["Npar"];
    int atomicZ = 1;
    double NperBunch = input1.bunchPara["NperBunch"];    
    double* q = new double[nBeam]; // used to turn on the beam dynamics.
    double QperBunch; 
    double Ek;
    ...
    double q0 = QperBunch/nPar;
    double dqodn = QperBunch/nPar/n_q_ramp;

    double Rring = input1.ringPara["R"];
    double GMTSQ = input1.ringPara["GMTSQ"];
    ...
    int* index = new int[nPar*nBunch*nBeam];
    int* neighbor = new int[nPar*nBunch*nBeam];
    double* bunch = new double[nPar*2*nBunch*nBeam];// to store relative time and gamma.
    double *temp = new double[nPar*nBunch*nBeam*2];
    int* parFlag = new int[nPar*nBunch*nBeam]; // flag that marks the partcle: 1 means under tracking, 0 means lost
    int* nPar_survive = new int[nBunch*nBeam];
    double* M1 = new double[nBunch*2*nBeam]; // to store the centroids of each bunch. 
    double* M2 = new double[nBunch*3*nBeam]; // to store the centroids of each bunch. 
    double* weight = new double[nPar*nBunch*nBeam];
    double* first_archive = new double[(N-n_fill)*2];    
    double omega0 = 2*pi*c_light*sqrt(1-1/Gamma0/Gamma0)/(2*pi*Rring);
    double Trev = 2*pi/omega0;
    double Prad = input1.ringPara["Prad"];
    double Urad0 = Prad/(QperBunch*nBunch/Trev);

    int* h = new int[nRF1+nRFc+nRF2];
    int* delay = new int[nRF1+nRFc+nRF2];
    int* nSmp = new int[nRF1+nRFc+nRF2];
    int* fb_on = new int[nRF1+nRFc+nRF2];
    ...
    int main_detune = input1.generalPara["main_detune"];
    double* omegac = new double[nRF1+nRFc+nRF2];
    double* QL = new double[nRF1+nRFc+nRF2];
    double* RoQ = new double[nRF1+nRFc+nRF2];
    double* R = new double[nRF1+nRFc+nRF2];
    double* C = new double[nRF1+nRFc+nRF2];//[2] = {QL[0]/R[0]/omegac[0],QL[1]/R[1]/omegac[1]};//9.2755373928124199e-11;
    double* Leff = new double[nRF1+nRFc+nRF2];//[2] = {dt*dt/(1-cos(dt*omegac[0]))/2/C[0],dt*dt/(1-cos(dt*omegac[1]))/2/C[1]};
    int* on = new int[nRF1+nRFc+nRF2];
    double dt = 0;
    int stride = 1+2*nBeam+3*(nRF1+nRFc+nRF2);
    ...
    for (int i = 0;i<nRF1+nRFc+nRF2;++i){
        ...
        R[i] =QL[i]*RoQ[i];
        C[i] = QL[i]/R[i]/omegac[i];//[2] = {QL[0]/R[0]/omegac[0],QL[1]/R[1]/omegac[1]};//9.2755373928124199e-11;
        Leff[i] = dt*dt/(1-cos(dt*omegac[i]))/2/C[i];//[2] = {dt*dt/(1-cos(dt*omegac[0]))/2/C[0],dt*dt/(1-cos(dt*omegac[1]))/2/C[1]};
        V_I_ref[i] = input1.rfPara["Vref_I"][i];
        V_Q_ref[i] = input1.rfPara["Vref_Q"][i];  
        Ig_I_ref[i] = V_I_ref[i]/R[i];
        Ig_Q_ref[i] = V_Q_ref[i]/R[i];
    ...
    std::cout<<"n_detune_ramp : "<<n_detune_ramp<<std::endl;
    double Trf = 2*pi/omegarf[0]; // for bunch insertion only
    ...
    double Qs = sqrt(h[mainRF]*atomicZ*fabs(V_I_ref[mainRF])*eta*fabs(cos((Phi0-90)/180*M_PI))/(2*pi*Ek));

    double delta_hat = 0;
    switch(type_of_particle){
        case 0:
            delta_hat = sqrt(epsilon)*sqrt(omega0/(pi*Ek))*std::pow((h[mainRF]*atomicZ*fabs(V_I_ref[mainRF])*fabs(cos((Phi0-90)/180*M_PI))/(2*pi*Ek*fabs(eta))),0.25);
            break;
        case 1:
    ...
    double Ek_damp = Qs/input1.ringPara["Ek_damp"];
    ...
    double* store = new double[nData*(stride)*int(N/step)];
    double* store_f_cav = new double[int(N/step)*(nRF1+nRFc+nRF2)];
...
//#pragma omp parallel for 
    for(int i = 0;i<nData_tot;++i){
        ...
        for(int j = 0;j<nRF;++j){
            data1[i*stride+j*3+1+2*nBeam] = 0;//V_I_ref[j]*sin(data1[i*stride]*omegarf[j]);//0;
            data1[i*stride+j*3+1+2*nBeam+1] = Ig_I_ref[j]*sin(data1[i*stride]*omegarf[j])+Ig_Q_ref[j]*cos(data1[i*stride]*omegarf[j]);
            data1[i*stride+j*3+1+2*nBeam+2] = 0;//-V_I_ref[j]/omegarf[j]*cos(data1[i*stride]*omegarf[j]);
    ...
    get_init_t0s(t0s,fill_step*Trf,Trf/2,Trev,nBeam,nBunch,beam_shift,h[0]);
    srand(0);
    std::cout<<"Initializing bunches."<<std::endl;
    /*
    for( int i = 0;i<nBunch;++i){
        init_bunch(bunch,Trf/2+fill_count*fill_step*Trf,Gamma0,t_hat,delta_hat,nPar,nBunch,fill_count);
        fill_count++;
    }
    */
    for( int i = 0;i<nBunch*nBeam;++i){
        init_bunch(bunch,t0s[i],Gamma0,t_hat,delta_hat,nPar,nBunch,fill_count,nBeam);
        fill_count++;
    }
    ...
    for(int i = 0;i<N;++i){
        ...
        updateIb(bunch,data1,weight,dt,q,fill_count,nPar,nData_tot,delay[0]+nSmp[0],stride,nBeam, index,neighbor);
        
        updateVI(data1,k,nData_tot,dt,Ig_I_ref,Ig_Q_ref, V_I_ref, V_Q_ref,R,gii,gqq,I,Q,sintable, costable, tempI, tempQ, delay,nSmp,nRF,nRF1,nRFc,nRF2,stride,nBeam);
        memcpy((void*)&data1[0],&(data1[nData*stride]),sizeof(double)*(delay[0]+nSmp[0])*stride);

        if(type_of_particle==1){
            updateParticle_e(bunch,data1, weight, M1, Gamma0,eta,dt,Trev,Ek_damp,Urad0,coeff_decay_long, coeff_excite_long,fill_count,nPar,nRF, nHOM,index, neighbor);
        }
        else{
        ...
        update_M1(bunch,M1,nPar,fill_count,nPar_survive,parFlag);
        if(fill_count>0&i>=n_fill){
            first_archive[(i-n_fill)*2] = M1[0];
            first_archive[(i-n_fill)*2+1] = M1[fill_count];
        }
        if(i%step == 0){
    ...
    std::cout<<"GFLOPS : "<<168/(t_end-t_start)<<std::endl;
    return 0;
}
